...
export default function ProductList() {
  ...
  const [userName, setUserName] = useState("") // Actualización: userName ahora es un estado
  ...
  useEffect(() => {
    ...
    if (nombre_user) {
      // Actualización: Establecer userName desde localStorage
      setUserName(nombre_user)
    }
    ...
    <>
      ...
      {showingRecommendations && (
        <div className="mb-4 p-2 bg-purple-100 rounded-lg text-purple-800 text-sm flex items-center">
          <Sparkles className="h-4 w-4 mr-2" />
          Mostrando recomendaciones personalizadas basadas en tu historial, {userName || "usuario"}{" "}
          {/* Actualización: Uso de userName */}
          <Button
            variant="ghost"
            size="sm"
